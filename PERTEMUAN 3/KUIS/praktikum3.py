...
# Hide unused columns
for row in range(3):
    for col in range(4, 5):  # Hide column 5 (index 4)
...
def demonstrate_image_registration():
    ...
    # Manual registration attempt (simplified)
    # Find transformation using phase correlation (simplified)
    f_transform = np.fft.fft2(ref_img)
    g_transform = np.fft.fft2(moving_img)
    
    # Cross-power spectrum
    cross_power = (f_transform * np.conj(g_transform)) / np.abs(f_transform * np.conj(g_transform) + 1e-10)
    phase_corr = np.fft.ifft2(cross_power)
    phase_corr = np.abs(phase_corr)
    
    # Find peak
    peak_pos = np.unravel_index(np.argmax(phase_corr), phase_corr.shape)
    translation = (peak_pos[0] - ref_img.shape[0]//2, peak_pos[1] - ref_img.shape[1]//2)
    
    # Apply estimated translation
    M_estimated = np.float32([[1, 0, translation[1]], [0, 1, translation[0]]])
    ...
    axes[0, 2].set_title(f'Phase Correlation\nPeak at: {peak_pos}')
    axes[0, 2].axis('off')
    
    # Difference before registration
    diff_before = cv2.absdiff(ref_img, moving_img)
    axes[1, 0].imshow(diff_before, cmap='gray')
    axes[1, 0].set_title(f'Difference Before\nMean Diff: {np.mean(diff_before):.1f}')
    axes[1, 0].axis('off')
    
    # Registered image
    axes[1, 1].imshow(registered_img, cmap='gray')
    axes[1, 1].set_title(f'Registered Image\nEst. Translation: {translation}')
    axes[1, 1].axis('off')
    
    # Difference after registration
    diff_after = cv2.absdiff(ref_img, registered_img)
    axes[1, 2].imshow(diff_after, cmap='gray')
    axes[1, 2].set_title(f'Difference After\nMean Diff: {np.mean(diff_after):.1f}')
    axes[1, 2].axis('off')
    
    plt.suptitle('Image Registration menggunakan Phase Correlation', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.show()
    
    return ref_img, moving_img, registered_img

# Run image registration demonstration
demonstrate_image_registration()

print("\n=== PRAKTIKUM 3 SELESAI ===")
print("Output visual telah ditampilkan. Silakan analisis hasil transformasi.")